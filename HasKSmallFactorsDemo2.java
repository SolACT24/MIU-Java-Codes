/* Given a positive integer k, another positive integer n is said to have k-small factors if n can be written as 
a product u*v where u and v are both less than k. For instance, 20 has 10-small factors since both 4 and 5 are 
less than 10 and 4*5 = 20. (For the same reason, it is also true to say that 20 has 6-small factors, 7-small factors,
...
*/
public class HasKSmallFactorsDemo2 //Perfect Answer!
{
    ...
    {
        for(int i = 1; i <= n; i++)
            if(n % i == 0 && n/i < k && i < k) return true; //{System.out.println(n/i+" and "+i); return true;}
