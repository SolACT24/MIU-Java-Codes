...
*/
public class NUniqueDemo2 //Perfect Answer!
{
    public static void main(String[] args)
    {
        System.out.println(isNUnique(new int[] {7,3,3,2,4}, 6)); //0
        System.out.println(isNUnique(new int[] {7,3,3,2,4}, 10)); //0
        System.out.println(isNUnique(new int[] {7,3,3,2,4}, 11)); //1
        System.out.println(isNUnique(new int[] {7,3,3,2,4}, 8)); //0
        System.out.println(isNUnique(new int[] {7,3,3,2,4}, 4)); //0
        System.out.println(isNUnique(new int[] {1}, 8)); //0
        System.out.println(isNUnique(new int[] {2,7,3,4}, 5)); //1
        System.out.println(isNUnique(new int[] {2,3,3,4}, 5)); //0
    }
    static int isNUnique(int[] a, int n)
    {
        ...
        int totalCount = 0;
        for(int i = 0; i < a.length-1; i++) //Since we want to compare with the last index in j, we have to make i < a.length-1
        {
            for(int j = i+1; j < a.length; j++) //Since j is started with i+1, so j < a.length
                if(n == a[i]+a[j]) totalCount++;
        }
