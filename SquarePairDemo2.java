...
public class SquarePairDemo2
{
    public static void main(String[] args)
    {
        ...
        //System.out.println(countSquarePairs(new int[] {2, 7, 2, 2})); //{2,7}, {7,2} & {7,2}. The square pairs must be unique!
    }
    static int countSquarePairs(int[] a)
    {
       ...
       for(int i = 0; i < a.length-1; i++)
       {
           for(int j = i+1; j < a.length; j++)
                if(a[i] > 0 && a[j] > 0 && a[i] != a[j] && isPerfectSquare(a[i]+a[j]) == 1) count++; //a[i] < a[j] || a[j] < a[i] ==> a[i] != a[j]
       }
       return count;
    }
