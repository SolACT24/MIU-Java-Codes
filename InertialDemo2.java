...
However, {12, 11, 4, 9, 2, 3, 10} is not inertial because it fails condition (c), i.e., 10 (which is even) is greater 9 (which is odd) 
but 10 is not the maximum value in the array.
...
public class InertialDemo2 
{
    ...
    {
        int countOdd = 0, maxEven = Integer.MIN_VALUE;
        ...
        if(countOdd == 0 || maxEven % 2 != 0) return 0;
        ...
        { 
            if(a[i] % 2 != 0)
            {
                for(int j = 0; j < a.length; j++)
                    if(a[j] % 2 == 0 && a[j] != maxEven && a[i] <= a[j]) return 0; //Here is the point!!  
            } 
        }
        return 1;
    }
