/* An array is defined to be a 235 array if the number of elements divisible by 2 plus the number of elements divisible by 3 plus 
the number of elements divisible by 5 plus the number of elements not divisible by 2, 3, or 5 is equal to the number of elements 
of the array. Write a method named is235Array that returns 1 if its array argument is a 235 array, otherwise it returns 0. 
...
public class _235ArrayDemo
{
    ...
    {
        int countDivBy2 = 0, countDivBy3 = 0, countDivBy5 = 0, countNotDivBy235 = 0;

        for(int i = 0; i < a.length; i++)
        {
            if(a[i] % 2 == 0) countDivBy2++; 
            if(a[i] % 3 == 0) countDivBy3++;
            if(a[i] % 5 == 0) countDivBy5++;
            if(a[i] % 2 != 0 && a[i] % 3 != 0 && a[i] % 5 != 0) countNotDivBy235++;
        }        
        if((countDivBy2 + countDivBy3 + countDivBy5 + countNotDivBy235) == a.length) return 1;
        else return 0;
    }
}